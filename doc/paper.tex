...
In section \ref{sec:construct} we describe the circuit constructions we use to reduce period finding into constant-sized gates, while tracking the number of required dirty ancilla.
...
We then use the available dirty ancilla to reduce the single remaining CNOT with many controls into Toffoli gates.
...
\begin{figure}
  ...
  \caption{\em
    ...
    Uses no ancilla, $O(c + n)$ gates, and $O(c + \lg n)$ depth.
  }
  \label{fig:multi-not}
\end{figure}
...
\begin{figure}
  ...
  \caption{\em
    When $c-2$ dirty ancilla are available, a controlled-not with $c$ controls can be reduced into $4c - 8$ Toffoli gates.
  }
...
The source code for the ProjectQ implementation, and an issue tracker for submitting errata, can be found at \href{https://github.com/Strilanc/PaperImpl-2017-DirtyPeriodFinding}{github.com/Strilanc/PaperImpl-2017-DirtyPeriodFinding}.
...
The reason for the increased cost is that using dirty ancilla results in more uncomputation work, in repeating some operations twice and conditionally to do them once unconditionally, and in less efficient constructions in general.
...
All the other constructions, for simpler arithmetic, can be done with two or fewer dirty ancilla.
...
Despite the large reduction in the number of ancilla available to the underlying arithmetic operations, they would still have more than enough ancilla and contribute no additional ancilla to the overall circuit.
